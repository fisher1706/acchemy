...
conn = create_engine('sqlite:///db_03_01.sqlite', echo=False)
...
if __name__ == '__main__':
    ...
    for i in range(3):
        ...
        for j in range(5):
            book = all_books[j]
            review = Review(book_id=book.id, text=f'Ставлю{j + 1} из 5')
            user.reviews.append(review)
        session.add(user)
        session.commit()

    books_by_title = session.query(Book).filter(Book.title == 'Властелин колец').all()
    ...
    books_5_score = session.query(Book).join(Review).filter(Review.text == 'Ставлю 5 из 5').all()
    print('Запрос с JOIN:', books_5_score)

    true_reviews = session.query(Review).join(User).filter(User.books.any(Book.id == Review.book_id)).all()
